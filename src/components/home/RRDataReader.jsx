...
function RRDataReader({ processData, setProcessData, dataChangeCounter }) {
    ...
    const handleUpload = (event) => {
        ...
        reader.onload = (function (theFile) {
            return function (e) {
                ...
                setProcessData({ data: dataHolder, tq: _tq })
            };
        })(file);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ minWidth: 100, padding: 20 }}>
                <span>TQ: {processData.tq}</span>
                {
                    processData?.data?.length === 0 && <h5>No data given.</h5>
                }
                {
                    processData?.data?.map(d => {
                        return (
